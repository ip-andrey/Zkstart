...
async def wrap_eth(account_id, key, proxy):
    ...
    await unwrap_eth(account_id, key, proxy)
...
async def swap_syncswap(account_id, key, proxy):
    ...
    from_token = "ETH"
    to_token = random.choice(["USDC","USDT","MAV"])
    ...
    await syncswap.swap(to_token, from_token, min_amount, max_amount, decimal, slippage, all_amount, min_percent=98,
                        max_percent=100)
...
async def swap_mute(account_id, key, proxy):
    ...
    from_token = "ETH"
    to_token = random.choice(["USDC", "WBTC"])
    ...
    await mute.swap(to_token, from_token, min_amount, max_amount, decimal, slippage, all_amount, min_percent=98,
                       max_percent=100)
...
async def swap_spacefi(account_id, key, proxy):
    ...
    from_token = "ETH"
    to_token = random.choice(["USDC","USDT","BUSD"])
    ...
    await spacefi.swap(to_token, from_token, min_amount, max_amount, decimal, slippage, all_amount, min_percent=98,
                       max_percent=100)
...
async def swap_pancake(account_id, key, proxy):
    ...
    await pancake.swap(to_token, from_token, min_amount, max_amount, decimal, slippage, all_amount, min_percent=98,
                     max_percent=100)
...
async def swap_woofi(account_id, key, proxy):
    ...
    await woofi.swap(to_token, from_token, min_amount, max_amount, decimal, slippage, all_amount, min_percent=98,
                        max_percent=100)
...
async def swap_odos(account_id, key, proxy):
    ...
    # from_token = "ETH"
    # to_token = random.choice(["USDC", "USDT", "BUSD" , "MAV"])
    ...
    await odos.swap(to_token, from_token, min_amount, max_amount, decimal, slippage, all_amount, min_percent=98,
                       max_percent=100)

async def volume_odos(account_id, key, proxy):
    """
    Make swap on Odos
    ______________________________________________________
    from_token – Choose SOURCE token ETH, WETH, USDC, USDT, BUSD, MAV, OT, MATIC, WBTC | Select one
    ...
    from_token = "USDC"
    to_token = random.choice([ "USDT", "BUSD" ])
    ...
    swaps = random.randint(3,5)

    for i in range(swaps):
        loguru.logger.info(f"Will make {swaps} swaps")
        await odos.swap(
            from_token, to_token, min_amount, max_amount, decimal, slippage, all_amount, min_percent, max_percent
        )
        await asyncio.sleep(random.randint(5, 10))
        ...
        loguru.logger.info(f"delay between swaps {delay}")

    await asyncio.sleep(random.randint(5, 10))

    await swap_odos(account_id, key, proxy)
...
async def swap_zkswap(account_id, key, proxy):
    ...
    await zkswap.swap(to_token, from_token, min_amount, max_amount, decimal, slippage, all_amount, min_percent=98,
                       max_percent=100)
...
async def swap_xyswap(account_id, key, proxy):
    ...
    await xyswap.swap(to_token, from_token, min_amount, max_amount, decimal, slippage, all_amount, min_percent=98,
                       max_percent=100)
...
async def swap_openocean(account_id, key, proxy):
    ...
    await openocean.swap(to_token, from_token, min_amount, max_amount, decimal, slippage, all_amount, min_percent=98,
                       max_percent=100)
...
async def swap_inch(account_id, key, proxy):
    ...
    await inch_dex.swap( to_token, from_token, min_amount, max_amount, decimal, slippage, all_amount, min_percent=98,
                        max_percent=100)
...
async def swap_maverick(account_id, key, proxy):
    ...
    await maverick.swap(to_token, from_token, min_amount, max_amount, decimal, slippage, all_amount, min_percent=98,
                       max_percent=100)
...
async def swap_vesync(account_id, key, proxy):
    ...
    await vesync.swap(to_token, from_token, min_amount, max_amount, decimal, slippage, all_amount, min_percent=98,
                        max_percent=100)
...
async def custom_routes(account_id, key, proxy):
    ...
    ANOTHER:
        ...
        – create_safe
    ______________________________________________________
    Disclaimer - You can add apps to [] to select random ones,
    ...
    # use_modules = [[send_mail , deposit_eralend , deposit_basilisk , deposit_zerolend, enable_collateral_eralend , enable_collateral_basilisk]]
    use_modules = [ [send_mail, swap_inch , swap_syncswap , swap_woofi] , send_mail, send_mail ]
    # use_modules = [ [send_mail , wrap_eth] ,send_mail, send_mail , [swap_maverick , swap_pancake] ,[swap_odos,swap_vesync] ,  [swap_syncswap , swap_mute] , [deposit_zerolend ,deposit_eralend , deposit_basilisk] ]
    # use_modules = [ [swap_odos, swap_vesync],[swap_syncswap, swap_mute], [deposit_zerolend, deposit_eralend, deposit_basilisk]]
    # use_modules = [ volume_odos ]
    sleep_from = 50
    sleep_to = 100
